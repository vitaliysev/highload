...
import (
	...
	rediscache "marketplace/search/internal/cache/redis"
	"marketplace/search/internal/handler"
	repopg "marketplace/search/internal/repository/postgres"
...
func main() {
	...
	searchRepo := repopg.NewSearchRepo(db)
	searchCache := rediscache.NewSearchCache(rdb)
	svc := service.New(searchRepo, searchCache)
	h := handler.New(svc)
